...
typedef ap_axiu<32*2,4,5,5> AXI_VAL;//
//change to <64,4,5,5> from <32,4,5,5> since the struct float has 2 members
...
// function prototypes
void standalone_mmult (struct_float A[32][32], struct_float B[32][32], struct_float C[32][32]);
...
void matrix_multiply_ref(T a[DIM][DIM], T b[DIM][DIM], T out[DIM][DIM])
{
	...
	for (int ia = 0; ia < DIM; ++ia)
		for (int ib = 0; ib < DIM; ++ib)
		{
			...
			struct_float sum = {0,0};

			for (int id = 0; id < DIM; ++id){

				sum.data0 += a[ia][id].data0 * b[id][ib].data0;
				sum.data1 += a[ia][id].data1 * b[id][ib].data1;
			}
...
void mmult_hw(T a[DIM][DIM], T b[DIM][DIM], T out[DIM][DIM])
{
	...
	L1:for (int ia = 0; ia < DIM; ++ia)
		L2:for (int ib = 0; ib < DIM; ++ib)
		{
			...
			T sum = {0,0};//change the sum to struct_float and set 2 data as 0
...
#pragma HLS INLINE
	...
	assert(sizeof(T) == sizeof(int)*2);//change to 32*2 since the struct float has 2 members
	union
	{
		int ival;
		T oval;
...
#pragma HLS INLINE
	...
	assert(sizeof(T)*8 == 32*2);//change to 32*2 since the struct float has 2 members
